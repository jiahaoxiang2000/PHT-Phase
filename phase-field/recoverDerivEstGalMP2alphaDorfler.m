function [quadRef,estErrorGlob]=recoverDerivEstGalMP2alphaDorfler(PHTelem, GIFTmesh, u, quadList, p, q, Cmat, targetScale)
...
%                 hold on
                ...
                stressvect = Cmat*B'*u(dscrtx);%积分点上的应力
                
                %set-up linear system for solving for the recovered stress建立求解恢复应力的线性系统
...
quad_E_level = zeros(1,numQuads);
quadIndex = zeros(numQuads,2);
for indexPatch = 1:numPatches
    for j=1:size(quadList{indexPatch},1)
        quadCounter = quadCounter + 1;
        quadErrors(quadCounter) = sum(estError{indexPatch}(quadList{indexPatch}(j,:))); 
        quad_E_level(quadCounter) = PHTelem{indexPatch}(quadList{indexPatch}(j,1)).level;
...
quad_E_level = quad_E_level(:,index);
