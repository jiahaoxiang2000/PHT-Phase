function [ d ] = imposeDirichletd_threepoint(dstiff, drhs, PHTelem, p, q,sizeBasis)
...
for patchIndex=1:length(PHTelem)
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
          ...
          if (patchIndex==10) && (PHTelem{patchIndex}(i).vertex(3)==1)
               bcdof_mid=[bcdof_mid,PHTelem{patchIndex}(i).nodesGlobal(right_nodes)];
          end
          if (patchIndex==11) && (PHTelem{patchIndex}(i).vertex(1)==0)
