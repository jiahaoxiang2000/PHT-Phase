...
%calculate regions of a (p+1)*(q+1) matrix 计算一个（p+1）*(q+1)的矩阵区域

alpha = floor((p-1)/2); %floor向下取整
beta = floor((q-1)/2);


%form the index matrix 形成指标矩阵
%indexMatrix = reshape(1:(p+alpha)*(q+beta),q+alpha,p+beta)';

indexMatrix = reshape(1:(p+1)*(q+1),q+1,p+1)';

%pick the corners 选择块
