function  plotSolPHTElasticMPVM22( u, PHTelem, GIFTmesh, p, q, Cmat, factor )
        ...
        if isempty(PHTelem{indexPatch}(i).children)
            ...
            for jj=1:numPts
                for ii=1:numPts
                    ...
                    dispmatx(jj,ii) = dispmatx(jj,ii) + cR'*u(scrt_x);
                    dispmaty(jj,ii) = dispmaty(jj,ii) + cR'*u(scrt_y);
                    
                    %calculate the stress values
                    stressvect{jj,ii} = Cmat*B'*u(dscrtx);
