...
for patchIndex = 1:numPatches
    for i=1:length(PHTelem{patchIndex})
        if isempty(PHTelem{patchIndex}(i).children)
            ...
            for j=1:numPts
                coord_south(j,:) = paramMap(PHTmesh{patchIndex}, uref(j), -1, pxmin, pymin, pxmax, pymax );
                coord_east(j,:) = paramMap(PHTmesh{patchIndex}, 1, vref(j), pxmin, pymin, pxmax, pymax );
                coord_north(j,:) = paramMap(PHTmesh{patchIndex}, -uref(j), 1, pxmin, pymin, pxmax, pymax );
                coord_west(j,:) = paramMap(PHTmesh{patchIndex}, -1, -vref(j), pxmin, pymin, pxmax, pymax );
            end
