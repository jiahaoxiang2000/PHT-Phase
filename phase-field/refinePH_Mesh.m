function [patchList, PHTelem, dimBasis] = refinePH_Mesh(MP_patchRef, patchList, PHTelem, p, q, dimBasis)
%refines the patches marked by patchRef

numElem = length(PHTelem);
indexPatch = find(MP_patchRef > 0);
...
MP_patchRef = MP_patchRef(sortedIndex);

patchCounter = 0;
for i=1:length(MP_patchRef)
    if MP_patchRef(i)
