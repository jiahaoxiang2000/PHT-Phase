...
%inserts cross in PHTelem at elm_index   在单元插入交叉节点
%dimBasis: dimension of the PHT space   PHT空间维数


%define corner indices 定义块的指标
...
for e=elm_index    
    ...
    %add children elements and their easy neighbors添加子元素和它们的近邻(左下)子元素
    %lower-left child
    ...
    %T-junctions 在细化元素之外添加邻居，考虑到t连接
    ...
    %handle the removed T-junctions 处理移除的T接头
    dimBasisTemp = dimBasis;
    for ijunct = RTjunct
                
        %update the neighbors of the neighbor with children elements so we
        %get correct knotUln, knotUrn, knotVdn, knotVun用子元素更新邻居的邻居，这样我们得到正确的knotU1n, knotUrn, knotVdn, knotVun
        switch ijunct
            case 1                
        ...
        %update the Bezier extraction operators and element_nod indices更新Bezier提取操作符和element_nod索引        
    ...
    %of the children elements计算新的Bezier提取操作符和子元素的element_nod索引
    C_temp = PHTelem(e).C;
    ...
    %update the neighbors of the neighbors with self用self更新邻居的邻居
    for ichild = 1:4
