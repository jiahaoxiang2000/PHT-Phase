...
GIFTmesh = init2DGeometry_two_cracksGIFTMP(numPatches,L,H);
...
%l=0.0075;
l=(4*0.25)/(2.^level)+0.0021875;
u_inc =1e-5*ones(1000,1);% [1e-4*ones(70,1);1e-5*ones(800,1)];
...
%%  assemble the linear system
 % crack phase field solution（裂纹相场解）
    ...
    [ d ] = imposeDirichletd_two_cracks(dstiff, drhs, PHTelem, p, q,sizeBasis);
    %d = dstiff\drhs;
    %d(d>1)=1;d(d<0)=0;
    %u计算
    disp('Assembling the linear system...')
    ...
    [u,Fload,Uload ] = imposeDirichletu_two_cracks(ustiff, PHTelem, p, q,sizeBasis,ij,u_inc,Fload,Uload,u);
         toc
    disp('Plotting the solution...')
    if mod(ij,100)==0||ij==1
