...
GIFTmesh = init2DGeometry_two_XIEcracksGIFTMP(numPatches,L,H);
...
load('two_crack_patchBoundary.mat');
tic %启动秒表计时器\
keep_refining = 1;
%keep_E_refining = 1;
num_steps = 0;
...
%开始进行位移和d的交错更新
%step=1500;
%l=(4*10)/(2.^level)-0.125;
l=0.5;

u_inc = [1e-4*ones(600,1);1e-5*ones(3400,1)];
%u_inc = 1e-4*ones(600,1);
...
%%  assemble the linear system
 % crack phase field solution（裂纹相场解）
    ...
    [ d ] = imposeDirichletd_two_XIEcracks(dstiff, drhs, PHTelem, p, q,sizeBasis);
    %d = dstiff\drhs;
    %d(d>1)=1;d(d<0)=0;
    %u计算
    disp('Assembling the linear system...')
    ...
    [u,Fload,Uload ] = imposeDirichletu_two_XIEcracks(ustiff, PHTelem, p, q,sizeBasis,ij,u_inc,Fload,Uload,u);
     disp('Plotting the solution...')
    if mod(ij,1000)==0||ij==600
